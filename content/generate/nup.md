...
* Create `outFile` for a given [paper size](/paper) and arrange `imageFiles` on pages in a grid like fashion.
...
The following table lists all supported values for `n` and the resulting grid layout with respect to the orientation of the chosen output [paper size](/paper). See [here](https://en.wikipedia.org/wiki/N-up) for a thorough explanation of N-up.
...
| formsize, paper size | [paper size](/paper) to be used. Append L or P to enforce landscape/portrait mode| A4
| orientation          | one of `rd, dr, ld, dl` for PDF input files | rd
| enforce              | on/off true/false                           | on
| border               | on/off true/false                           | on
| margin               | float >= 0                                  | 0
| backgroundcolor, bgcol| [color](/getting_started/color) | none
...
Generate `out.pdf` using `A4L` landscape mode where each page fits 4 images onto a 2x2 grid.
The grid element border lines are rendered by default, and the default margin of 3 points is applied:
...
A single image input file supplied will produce a single page PDF output file.<br>
