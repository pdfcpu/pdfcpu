...
func TestAlternatingPageNumbersViaWatermarkMapLowLevel(t *testing.T) {
	...
	for i := 2; i <= ctx.PageCount; i++ {
		...
		desc := fmt.Sprintf("font:%s, points:12, sc:1 abs, pos:%s, off:%d 10, fillcol:%s, rot:0", fontName, pos, dx, fillCol)
		wm, err := api.TextWatermark(text, desc, true, false, units)
		if err != nil {
			t.Fatalf("%s: %v\n", msg, err)
		}
		m[i] = wm
	...
	// Add a stamp with the creation date on the center of the bottom of every page.
	text := fmt.Sprintf("Creation date: %v", time.Now().Format("2006-01-02 15:04"))
	wm, err := api.TextWatermark(text, "fo:Roboto-Regular, points:12, sc:1 abs, pos:bc, off:0 10, rot:0", true, false, units)
	if err != nil {
		t.Fatalf("%s %s: %v\n", msg, outFile, err)
	}
	...
	// Add a "Draft" stamp with opacity 0.6 along the 1st diagonale in light blue using Courier.
	wm, err = api.TextWatermark("Draft", "fo:Courier, sc:.9, fillcol:#00aacc, op:.6", true, false, units)
	if err != nil {
		t.Fatalf("%s %s: %v\n", msg, outFile, err)
	}
	if err := ctx.AddWatermarks(nil, wm); err != nil {
...
func TestImagesTextAndPDFWMViaWatermarkMap(t *testing.T) {
	...
	for i := 1; i <= pageCount; i++ {
		if i <= len(fileNames) {
			wm, err := api.ImageWatermark(fileNames[i-1], "pos:bl, sc:.25, rot:0", true, false, units)
			if err != nil {
				t.Fatalf("%s: %v\n", msg, err)
			}
			m[i] = wm
		...
		if i%2 > 0 {
			wm, err := api.PDFWatermark(inFile+":1", "sc:.25, pos:br, rot:0", true, false, units)
			if err != nil {
				t.Fatalf("%s: %v\n", msg, err)
			}
			m[i] = wm
		...
		wm, err := api.TextWatermark("Even page number", "rot:0", true, false, units)
		if err != nil {
			t.Fatalf("%s: %v\n", msg, err)
		}
		m[i] = wm
