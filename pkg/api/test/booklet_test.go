...
func TestBooklet(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{"TestBookletFromPDF8Up",
			...
			nil,
			"p:A3, enforce:on, g:on", // need to enforce orientation for 8up because portrait pdf input gets rotated onto sheet
			"points",
			8,
			false,
		},
		{"TestBookletFromPDF8UpPortraitShort",
			...
			nil,
			"p:A3, enforce:on, binding:short, g:on",
			"points",
			8,
			false,
		},
		{"TestBookletFromPDF8UpLandscapeLong",
			...
			nil,
			"p:A3, enforce:on, binding:long, g:on",
			"points",
			8,
			false,
		},
