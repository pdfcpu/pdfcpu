...
func validateArrayEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version, validate func(types.Array) bool) (types.Array, error) {
	...
	if o == nil {
		if required {
			return nil, fmt.Errorf("validateArrayEntry: dict=%s required entry=%s is nil", dictName, entryName)
		}
		if log.ValidateEnabled() {
	...
	a, ok := o.(types.Array)
	if !ok {
		return nil, fmt.Errorf("validateArrayEntry: dict=%s entry=%s invalid type %T", dictName, entryName, o)
	}

	// Validation
	if validate != nil && !validate(a) {
		return nil, fmt.Errorf("validateArrayEntry: dict=%s entry=%s invalid dict entry", dictName, entryName)
	}
...
func validateBooleanEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version, validate func(bool) bool) (*bool, error) {
	...
	if o == nil {
		if required {
			return nil, fmt.Errorf("validateBooleanEntry: dict=%s required entry=%s missing", dictName, entryName)
		}
		if log.ValidateEnabled() {
	...
	if !ok {
		return nil, fmt.Errorf("validateBooleanEntry: dict=%s entry=%s invalid type", dictName, entryName)
	}

	// Validation
	if validate != nil && !validate(b.Value()) {
		return nil, fmt.Errorf("validateBooleanEntry: dict=%s entry=%s invalid name dict entry", dictName, entryName)
	}
...
func validateBooleanArrayEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version, validate func(types.Array) bool) (types.Array, error) {
	...
	for i, o := range a {
		...
		if _, ok := o.(types.Boolean); !ok {
			return nil, fmt.Errorf("validateBooleanArrayEntry: dict=%s entry=%s invalid type at index %d\n", dictName, entryName, i)
		}
...
func timeOfDateObject(xRefTable *model.XRefTable, o types.Object, sinceVersion model.Version) (*time.Time, error) {
	...
	if !ok {
		return nil, fmt.Errorf("pdfcpu: validateDateObject: <%s> invalid date", s)
	}
...
func validateDateObject(xRefTable *model.XRefTable, o types.Object, sinceVersion model.Version) (string, error) {
	...
	t, ok := types.DateTime(s, xRefTable.ValidationMode == model.ValidationRelaxed)
	if !ok {
		return "", fmt.Errorf("pdfcpu: validateDateObject: <%s> invalid date", s)
	}
...
func validateDateEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version) (*time.Time, error) {
	...
	if s == "" {
		if required {
			return nil, fmt.Errorf("validateDateEntry: dict=%s required entry=%s is nil", dictName, entryName)
		}
		if log.ValidateEnabled() {
	...
	if !ok {
		return nil, fmt.Errorf("pdfcpu: validateDateEntry: <%s> invalid date", s)
	}
...
func validateDictEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version, validate func(types.Dict) bool) (types.Dict, error) {
	...
	if o == nil {
		if required {
			return nil, fmt.Errorf("validateDictEntry: dict=%s required entry=%s is nil", dictName, entryName)
		}
		if log.ValidateEnabled() {
	...
	d, ok := o.(types.Dict)
	if !ok {
		return nil, fmt.Errorf("validateDictEntry: dict=%s entry=%s invalid type", dictName, entryName)
	}

	// Validation
	if validate != nil && len(d) > 0 && !validate(d) {
		return nil, fmt.Errorf("validateDictEntry: dict=%s entry=%s invalid dict entry", dictName, entryName)
	}
...
func validateFloat(xRefTable *model.XRefTable, o types.Object, validate func(float64) bool) (*types.Float, error) {
	...
	if validate != nil && !validate(f.Value()) {
		return nil, fmt.Errorf("pdfcpu: validateFloat: invalid float: %s\n", f)
	}
...
func validateFunctionOrArrayOfFunctionsEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version) error {
	...
	if o == nil {
		if required {
			return fmt.Errorf("pdfcpu: validateFunctionOrArrayOfFunctionsEntry: dict=%s required entry=%s is nil", dictName, entryName)
		}
		if log.ValidateEnabled() {
...
func validateIndRefEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version) (*types.IndirectRef, error) {
	...
	if !ok {
		return nil, fmt.Errorf("pdfcpu: validateIndRefEntry: dict=%s entry=%s invalid type", dictName, entryName)
	}
...
func validateIndRefArrayEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version, validate func(types.Array) bool) (types.Array, error) {
	...
	for i, o := range a {
		...
		if _, ok := o.(types.IndirectRef); !ok {
			return nil, fmt.Errorf("pdfcpu: validateIndRefArrayEntry: invalid type at index %d\n", i)
		}
	}
...
func validateInteger(xRefTable *model.XRefTable, o types.Object, validate func(int) bool) (*types.Integer, error) {
	...
	if validate != nil && !validate(i.Value()) {
		return nil, fmt.Errorf("pdfcpu: validateInteger: invalid integer: %s\n", i)
	}
...
func validateIntegerEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version, validate func(int) bool) (*types.Integer, error) {
	...
	if o == nil {
		if required {
			return nil, fmt.Errorf("pdfcpu: validateIntegerEntry: dict=%s required entry=%s is nil", dictName, entryName)
		}
		if log.ValidateEnabled() {
	...
	i, ok := o.(types.Integer)
	if !ok {
		return nil, fmt.Errorf("pdfcpu: validateIntegerEntry: dict=%s entry=%s invalid type", dictName, entryName)
	}

	// Validation
	if validate != nil && !validate(i.Value()) {
		return nil, fmt.Errorf("pdfcpu: validateIntegerEntry: dict=%s entry=%s invalid dict entry", dictName, entryName)
	}
...
func validateIntegerArray(xRefTable *model.XRefTable, o types.Object) (types.Array, error) {
	...
	for i, o := range a {
		...
		default:
			return nil, fmt.Errorf("pdfcpu: validateIntegerArray: invalid type at index %d\n", i)
		}
...
func validateIntegerArrayEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version, validate func(types.Array) bool) (types.Array, error) {
	...
	for i, o := range a {
		...
		if _, ok := o.(types.Integer); !ok {
			return nil, fmt.Errorf("pdfcpu: validateIntegerArrayEntry: dict=%s entry=%s invalid type at index %d\n", dictName, entryName, i)
		}
...
func validateName(xRefTable *model.XRefTable, o types.Object, validate func(string) bool) (*types.Name, error) {
	...
	if validate != nil && !validate(name.Value()) {
		return nil, fmt.Errorf("pdfcpu: validateName: invalid name: %s\n", name)
	}
...
func validateNameEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version, validate func(string) bool) (*types.Name, error) {
	...
	if o == nil {
		if required {
			return nil, fmt.Errorf("pdfcpu: validateNameEntry: dict=%s required entry=%s is nil", dictName, entryName)
		}
		if log.ValidateEnabled() {
	...
	name, ok := o.(types.Name)
	if !ok {
		return nil, fmt.Errorf("pdfcpu: validateNameEntry: dict=%s entry=%s invalid type %T", dictName, entryName, o)
	}

	// Validation
	v := name.Value()
	if validate != nil && (required || len(v) > 0) && !validate(v) {
		return &name, fmt.Errorf("pdfcpu: validateNameEntry: dict=%s entry=%s invalid dict entry: %s", dictName, entryName, v)
	}
...
func validateNameArray(xRefTable *model.XRefTable, o types.Object) (types.Array, error) {
	...
	for i, o := range a {
		...
		if _, ok := o.(types.Name); !ok {
			return nil, fmt.Errorf("pdfcpu: validateNameArray: invalid type at index %d\n", i)
		}
...
func validateNameArrayEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version, validate func(a types.Array) bool) (types.Array, error) {
	...
	for i, o := range a {
		...
		if _, ok := o.(types.Name); !ok {
			return nil, fmt.Errorf("pdfcpu: validateNameArrayEntry: dict=%s entry=%s invalid type at index %d\n", dictName, entryName, i)
		}
...
func validateNumberEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version, validate func(f float64) bool) (types.Object, error) {
	...
	if validate != nil && !validate(f) {
		return nil, fmt.Errorf("pdfcpu: validateFloatEntry: dict=%s entry=%s invalid dict entry", dictName, entryName)
	}
...
func validateNumberArray(xRefTable *model.XRefTable, o types.Object) (types.Array, error) {
	...
	for i, o := range a {
		...
		default:
			return nil, fmt.Errorf("pdfcpu: validateNumberArray: invalid type at index %d\n", i)
		}
...
func validateNumberArrayEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version, validate func(types.Array) bool) (types.Array, error) {
	...
	for i, o := range a {
		...
		default:
			return nil, fmt.Errorf("pdfcpu: validateNumberArrayEntry: invalid type at index %d\n", i)
		}
...
func validateRectangleEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version, validate func(types.Array) bool) (types.Array, error) {
	...
	if validate != nil && !validate(a) {
		return nil, fmt.Errorf("pdfcpu: validateRectangleEntry: dict=%s entry=%s invalid rectangle entry", dictName, entryName)
	}
...
func validateStreamDictEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version, validate func(types.StreamDict) bool) (*types.StreamDict, error) {
	...
	if o == nil {
		...
		if xRefTable.ValidationMode == model.ValidationStrict {
			return nil, fmt.Errorf("pdfcpu: validateStreamDictEntry: dict=%s optional entry=%s is corrupt", dictName, entryName)
		}
		delete(d, entryName)
	...
	if sd == nil {
		if required {
			return nil, fmt.Errorf("pdfcpu: validateStreamDictEntry: dict=%s required entry=%s is nil", dictName, entryName)
		}
		if log.ValidateEnabled() {
	...
	if validate != nil && !validate(*sd) {
		return nil, fmt.Errorf("pdfcpu: validateStreamDictEntry: dict=%s entry=%s invalid dict entry", dictName, entryName)
	}
...
func decodeString(o types.Object, dictName, entryName string) (s string, err error) {
	...
	default:
		err = fmt.Errorf("pdfcpu: decodeString: dict=%s entry=%s invalid type %T", dictName, entryName, o)
	}
	return s, err
}
...
func validateStringEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version, validate func(string) bool) (*string, error) {
	...
	if o == nil {
		if required {
			return nil, fmt.Errorf("pdfcpu: validateStringEntry: dict=%s required entry=%s is nil", dictName, entryName)
		}
		if log.ValidateEnabled() {
	...
	if validate != nil && (required || len(s) > 0) && !validate(s) {
		return nil, fmt.Errorf("pdfcpu: validateStringEntry: dict=%s entry=%s invalid dict entry", dictName, entryName)
	}
...
func validateStringArrayEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version, validate func(types.Array) bool) (types.Array, error) {
	...
	for i, o := range a {
		...
		default:
			return nil, fmt.Errorf("pdfcpu: validateStringArrayEntry: invalid type at index %d\n", i)
		}
...
func validateArrayArrayEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version, validate func(types.Array) bool) (types.Array, error) {
	...
	for i, o := range a {
		...
		default:
			return nil, fmt.Errorf("pdfcpu: validateArrayArrayEntry: invalid type at index %d\n", i)
		}
...
func validateStringOrStreamEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version) error {
	...
	if o == nil {
		if required {
			return fmt.Errorf("pdfcpu: validateStringOrStreamEntry: dict=%s required entry=%s is nil", dictName, entryName)
		}
		if log.ValidateEnabled() {
	...
	default:
		return fmt.Errorf("pdfcpu: validateStringOrStreamEntry: dict=%s entry=%s invalid type", dictName, entryName)
	}
...
func validateNameOrStringEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version) error {
	...
	if o == nil {
		if required {
			return fmt.Errorf("pdfcpu: validateNameOrStringEntry: dict=%s required entry=%s is nil", dictName, entryName)
		}
		if log.ValidateEnabled() {
	...
	default:
		return fmt.Errorf("pdfcpu: validateNameOrStringEntry: dict=%s entry=%s invalid type", dictName, entryName)
	}
...
func validateIntOrStringEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version) error {
	...
	if o == nil {
		if required {
			return fmt.Errorf("pdfcpu: validateIntOrStringEntry: dict=%s required entry=%s is nil", dictName, entryName)
		}
		if log.ValidateEnabled() {
	...
	default:
		return fmt.Errorf("pdfcpu: validateIntOrStringEntry: dict=%s entry=%s invalid type", dictName, entryName)
	}
...
func validateBooleanOrStreamEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version) error {
	...
	if o == nil {
		if required {
			return fmt.Errorf("pdfcpu: validateBooleanOrStreamEntry: dict=%s required entry=%s is nil", dictName, entryName)
		}
		if log.ValidateEnabled() {
	...
	default:
		return fmt.Errorf("pdfcpu: validateBooleanOrStreamEntry: dict=%s entry=%s invalid type", dictName, entryName)
	}
...
func validateStreamDictOrDictEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version) error {
	...
	if o == nil {
		if required {
			return fmt.Errorf("pdfcpu: validateStreamDictOrDictEntry: dict=%s required entry=%s is nil", dictName, entryName)
		}
		if log.ValidateEnabled() {
	...
	default:
		return fmt.Errorf("pdfcpu: validateStreamDictOrDictEntry: dict=%s entry=%s invalid type", dictName, entryName)
	}
...
func validateIntegerOrArrayOfInteger(xRefTable *model.XRefTable, o types.Object, dictName, entryName string) error {
	...
	case types.Array:
		...
		for i, o := range o {
			...
			if _, ok := o.(types.Integer); !ok {
				return fmt.Errorf("pdfcpu: validateIntegerOrArrayOfInteger: dict=%s entry=%s invalid type at index %d\n", dictName, entryName, i)
			}

		}

	default:
		return fmt.Errorf("pdfcpu: validateIntegerOrArrayOfInteger: dict=%s entry=%s invalid type", dictName, entryName)
	}
...
func validateIntegerOrArrayOfIntegerEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version) error {
	...
	if o == nil {
		if required {
			return fmt.Errorf("pdfcpu: validateIntegerOrArrayOfIntegerEntry: dict=%s required entry=%s is nil", dictName, entryName)
		}
		if log.ValidateEnabled() {
...
func validateNameOrArrayOfName(xRefTable *model.XRefTable, o types.Object, dictName, entryName string) error {
	...
	case types.Array:
		...
		for i, o := range o {
			...
			if _, ok := o.(types.Name); !ok {
				err = fmt.Errorf("pdfcpu: validateNameOrArrayOfName: dict=%s entry=%s invalid type at index %d\n", dictName, entryName, i)
				return err
			}
	...
	default:
		return fmt.Errorf("pdfcpu: validateNameOrArrayOfName: dict=%s entry=%s invalid type", dictName, entryName)
	}
...
func validateNameOrArrayOfNameEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version) error {
	...
	if o == nil {
		if required {
			return fmt.Errorf("pdfcpu: validateNameOrArrayOfNameEntry: dict=%s required entry=%s is nil", dictName, entryName)
		}
		if log.ValidateEnabled() {
...
func validateBooleanOrArrayOfBoolean(xRefTable *model.XRefTable, o types.Object, dictName, entryName string) error {
	...
	case types.Array:
		...
		for i, o := range o {
			...
			if _, ok := o.(types.Boolean); !ok {
				return fmt.Errorf("pdfcpu: validateBooleanOrArrayOfBoolean: dict=%s entry=%s invalid type at index %d\n", dictName, entryName, i)
			}

		}

	default:
		return fmt.Errorf("pdfcpu: validateBooleanOrArrayOfBoolean: dict=%s entry=%s invalid type", dictName, entryName)
	}
...
func validateBooleanOrArrayOfBooleanEntry(xRefTable *model.XRefTable, d types.Dict, dictName, entryName string, required bool, sinceVersion model.Version) error {
	...
	if o == nil {
		if required {
			return fmt.Errorf("pdfcpu: validateBooleanOrArrayOfBooleanEntry: dict=%s required entry=%s is nil", dictName, entryName)
		}
		if log.ValidateEnabled() {
