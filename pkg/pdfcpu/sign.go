...
// 	// Validate the certificate chain provided in the signature:
// 	// Check that the signer's certificate is issued by a trusted Certificate Authority (CA) or can be traced back to a root CA.
// 	// Ensure that the certificate is still valid and has not been revoked (via mechanisms such as CRL or OCSP).
// 	// Verify the certificate's expiration date and ensure that it covers the signing time.
// 	// Check that the signature timestamp is within the validity period of the certificate.
// 	// 8. Timestamp Verification (Optional)
// 	// Some signatures include a trusted timestamp to confirm the signing time. This prevents future tampering even if the certificate has expired or been revoked after the signing date.
// 	// If a timestamp is present, validate its correctness and integrity by checking the Time Stamp Authority's (TSA) certificate.
...
// 	// PAdes profiles:
// 	// 3. ??? Long term validation (ETSI EN 319 142-1 (PAdES)
// 	// 4. ??? Document timestamp dict (DTS)(ETSI EN 319 142-1 (PAdES)

// 	// PAdES-E-BES	(Basic Electronic Signature) 			compatible with ETSI EN 319 122-2 CAdES-E-BES
// 	// ETSI EN 319 142-2

// 	// PAdES-E-EPES (Explicit Policy Electronic Signature)	compatible with ETSI EN 319 122-2 CAdES-E-EPES
// 	// ETSI EN 319 142-2

// 	// CMS Advanced electronic signature
// 	// a DER-encoded CMS SignedData binary data object containing the signature.
// 	//
// 	// The attributes in the SignedData object:
// 	//
...
// 	//    The timestamp should be applied on the digital signature immediately after the signature is created
// 	//    so the timestamp specifies a time as close as possible to the time at which the document was signed. The rules from clause 5.3 in ETSI EN 319 122-1 shall apply.
...
// 	// f) signing-certificate or signing-certificate-v2:
// 	//    shall be used as a signed attribute as described in the ESS signing-certificate attribute or as described in the ESS signing-certificate-v2 attribute as defined in clause 5.2.2 in ETSI EN 319 122-1.
// 	//    The details of the signing certificate attribute can be found in Internet RFC 5035.
// 	// g) signing-time: may be present. If present, it contains a UTC time. If the signng-time attiribute is present, the time of signing shall not be indicated by the value of the M entry in the signature dictionary.
...
// 	// 1.
// 	// A signature handler shall compare the hash value of the signer's certificate, with the hash value given in the signing-certificate
// 	// attribute or the signing-certificate-v2 attribute.
// 	// If the hashes do not match, then the signature is considered invalid.
// 	//
// 	// The signature handler shall use the public key contained in the signer's certificate to verify that the document digest
// 	// found in the signature is correctly signed.
// 	// If this is not the case, then the signature is considered invalid.

// 	// 2.
// 	// The signature handler shall validate the path of certificates used to verify the binding between the subject distinguished name
// 	// and subject public key as specified in RFC 5280 clause 6, using a set of validation parameters.
// 	// The signature may be verified against a time other than the current time if all validation information (e.g. certificates and revocation information)
// 	// is known to have existed at that time (e.g. using DSS (see 12.8.4.3, "Document Security Store (DSS)"); using document timestamps,
// 	// (see 12.8.5, "Document timestamp (DTS) dictionary"); or a signature timestamp is present in the signature as an unsigned attribute
// 	// (see 12.8.3.4.3, "Attributes for PAdES signatures")).
// 	// Otherwise, the local current time converted into the UTC shall be used.and subject public key as specified in RFC 5280 clause 6,
// 	// using a set of validation parameters.
// 	// The signature may be verified against a time other than the current time if all validation information
// 	// (e.g. certificates and revocation information) is known to have existed at that time (e.g. using DSS (see 12.8.4.3, "Document Security Store (DSS)");
// 	// using document timestamps, (see 12.8.5, "Document timestamp (DTS) dictionary");
// 	// or a signature timestamp is present in the signature as an unsigned attribute (see 12.8.3.4.3, "Attributes for PAdES signatures")).
// 	// Otherwise, the local current time converted into the UTC shall be used.

// 	// 3.
// 	// When the signer's signature is verified against a time indicated in a timestamp token present in a Document timestamp dictionary,
// 	// then the signature handler shall also validate the path of certificates used to verify the timestamp unit's certificate,
// 	// as specified in RFC 5280 clause 6, using a set of validation parameters which may be different from the previous one.

// 	// 4.
// 	// The revocation status of the certification path shall be checked as specified in 12.8.3.4.6, "Signature revocation checking model for PAdES signatures".
