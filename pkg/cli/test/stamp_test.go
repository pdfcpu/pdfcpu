...
func testAddWatermarks(t *testing.T, msg, inFile, outFile string, selectedPages []string, mode, modeParm, desc string, onTop bool) {
	...
	switch mode {
	case "text":
		wm, err = pdfcpu.ParseTextWatermarkDetails(modeParm, desc, onTop, units)
	case "image":
		wm, err = pdfcpu.ParseImageWatermarkDetails(modeParm, desc, onTop, units)
	case "pdf":
		wm, err = pdfcpu.ParsePDFWatermarkDetails(modeParm, desc, onTop, units)
	}
	if err != nil {
		t.Fatalf("%s %s: %v\n", msg, outFile, err)
	}
...
func TestStampingLifecyle(t *testing.T) {
	...
	// Stamp all pages.
	wm, err := pdfcpu.ParseTextWatermarkDetails("Demo", "", onTop, units)
	if err != nil {
		t.Fatalf("%s %s: %v\n", msg, outFile, err)
	}
	...
	wm, err = pdfcpu.ParseTextWatermarkDetails("Confidential", "", onTop, units)
	if err != nil {
		t.Fatalf("%s %s: %v\n", msg, outFile, err)
	}
	wm.Update = true
	...
	wm, err = pdfcpu.ParseTextWatermarkDetails("Footer", "pos:bc, c:0.8 0 0, op:.6, rot:0", onTop, units)
	if err != nil {
		t.Fatalf("%s %s: %v\n", msg, outFile, err)
	}
