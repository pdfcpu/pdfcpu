...
func listAttachments(t *testing.T, msg, fileName string, want int) []string {
	t.Helper()
	list, err := Process(ListAttachmentsCommand(fileName, nil))
	if err != nil {
...
func TestAttachments(t *testing.T) {
	...
	if _, err := Process(AddAttachmentsCommand(fileName, "", files, nil)); err != nil {
	...
	if _, err := Process(ExtractAttachmentsCommand(fileName, outDir, nil, nil)); err != nil {
		t.Fatalf("%s extract all attachments: %v\n", msg, err)
	}

	// Extract 1 attachment.
	if _, err := Process(ExtractAttachmentsCommand(fileName, outDir, []string{"golang.pdf"}, nil)); err != nil {
		t.Fatalf("%s extract one attachment: %v\n", msg, err)
	}

	// Remove 1 attachment.
	if _, err := Process(RemoveAttachmentsCommand(fileName, "", []string{"golang.pdf"}, nil)); err != nil {
		t.Fatalf("%s remove one attachment: %v\n", msg, err)
	}
	listAttachments(t, msg, fileName, 3)

	// Remove all attachments.
	if _, err := Process(RemoveAttachmentsCommand(fileName, "", nil, nil)); err != nil {
