...
// TODO implement
func validateUsageDict(xRefTable *types.XRefTable, dict *types.PDFDict) (err error) {

	// see 8.11.4.4

	logInfoValidate.Println("*** writeUsage begin ***")

	err = errors.New("*** writeUsage: unsupported ***")

	logInfoValidate.Println("*** writeUsage end ***")

	return
}

// TODO implement
func validateOptionalContentGroupIntent(xRefTable *types.XRefTable, dict *types.PDFDict, dictName string, entryName string, required bool, sinceVersion types.PDFVersion) (err error) {

	logInfoValidate.Println("*** validateOptionalContentGroupIntent begin ***")

	err = errors.New("*** validateOptionalContentGroupIntent: unsupported ***")
...
func validateOptionalContentGroupDict(xRefTable *types.XRefTable, dict *types.PDFDict) (err error) {
	...
	// TODO Ausbauen!
	d, err := validateDictEntry(xRefTable, dict, dictName, "Usage", OPTIONAL, types.V10, nil)
	if err != nil {
		return
	}

	if d != nil {
		err = validateUsageDict(xRefTable, d)
		if err != nil {
			return
		}
	}
