...
// TODO implement
func validateThreadDict(xRefTable *types.XRefTable, obj interface{}) (err error) {

	logInfoValidate.Println("*** validateThreadDict begin ***")

	err = errors.New("*** validateThreadDict: not supported ***")
...
func validateThreads(xRefTable *types.XRefTable, rootDict *types.PDFDict, required bool, sinceVersion types.PDFVersion) (err error) {
	...
	if indRef == nil {
		if required {
			err = errors.Errorf("validateThreads: required entry \"Threads\" missing")
			return
		}
	...
	// Version check
	if xRefTable.Version() < sinceVersion {
		return errors.Errorf("validateThreads: unsupported in version %s.\n", xRefTable.VersionString())
	}

	for _, obj := range *arr {
		err = validateThreadDict(xRefTable, obj)
		if err != nil {
			return
		}
	}
...
func validateInfoObject(xRefTable *types.XRefTable) (err error) {

	logInfoValidate.Println("*** validateInfoObject begin ***")
	...
	if hasPieceInfo && !hasModDate {
		err = errors.Errorf("validateInfoObject: missing required entry \"ModDate\"")
		return
	}

	logInfoValidate.Println("*** validateInfoObject end ***")
...
func XRefTable(xRefTable *types.XRefTable) (err error) {
	...
	err = validateInfoObject(xRefTable)
	if err != nil {
		return
	}
